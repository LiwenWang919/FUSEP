...
data = dict(
    ...
    train=dict(
        type='SemiDataset',
        ann_file=data_root + f'annotations_json/voc07_trainval.json',
        ann_file_u=data_root + f'annotations_json/voc12_trainval.json',
    ...
    val=dict(
        type=dataset_type,
        ann_file=data_root + f'annotations_json/voc07_test.json',
...
labelmatch_hook_cfg = dict(
    samples_per_gpu=samples_per_gpu,
    workers_per_gpu=4,
    label_file=data_root + f'annotations_json/voc07_trainval.json',
    evaluation=dict(interval=update_interval, metric='bbox', by_epoch=False),
    data=dict(
        type='TXTDataset', img_prefix=data_root,
        ann_file=data_root + f'annotations_txt/voc12_trainval.txt',
...
log_level = 'INFO'
resume_from = None
load_from = f'./pretrained_model/baseline/voc.pth'
workflow = [('train', 1)]

# # -------------------------model------------------------------
model = dict(
    type='LabelMatch',
    ema_config='./configs/baseline/ema_config/baseline_voc.py',
    ema_ckpt=load_from,
    cfg=dict(
        debug=False,
    ),
