...
class Trainer():
    def __init__(self, opt):
        if opt.part == 'heart':
            ...
            opt.n_class = 9
        elif opt.part == 'CHAOS' or opt.part == 'MMWHS':
            opt.n_class = 5
        else:
        ...
        vaildset = COCO_Dataset(self.opt, operation='val')
        ...
        testset  = COCO_Dataset(self.opt, operation='test')
    ...
    def train(self):
        ...
        for epoch in range(self.opt.epoch):
            ...
            if eval_result['map'] > best_map and epoch >= 10:
    ...
    @torch.no_grad()
    def eval(self, dataloader, path=None, gt=False, test_num=10000):
        self.model.eval()
        ...
        features_tsne = {level: [] for level in range(5)}
        labels_tsne = {level: [] for level in range(5)}
        # features_tsne = []
        # labels_tsne = []
        # level = 3
        n_class = self.opt.n_class - 1 
        path = f'TSNE/tsne_{self.opt.part}_{time}'
        name = self.opt.selected_source_hospital + "_" + self.opt.selected_target_hospital

        for ids, (imgs, gt_targets, ids) in tqdm(enumerate(dataloader)):

            preds = {}
            imgs = imgs.tensors.to(device=self.opt.device)
            ...
            for level in range(5):
                label = torch.arange(n_class)
                cls_pred_level = cls_pred[level].squeeze(dim=0)
                cls_pred_level = cls_pred_level.cpu()
                cls_pred_level = cls_pred_level.view(cls_pred_level.shape[0], -1)
                features_tsne[level].append(cls_pred_level)
                labels_tsne[level].append(label)
            # label = torch.arange(n_class)
            # cls_pred_level = cls_pred[level].squeeze(dim=0)
            # cls_pred_level = cls_pred_level.cpu()
            # cls_pred_level = cls_pred_level.view(cls_pred_level.shape[0], -1)
            # features_tsne.append(cls_pred_level)
        ...
        for level, feature in features_tsne.items():
            label = labels_tsne[level]
            tsne(feature, label, level, path, name)
        # tsne(features_tsne, labels_tsne, level, path, name)
    ...
    def save(self, save_optimizer=False, save_path=None, **kwargs):
        ...
        if save_path is None:
            ...
            save_path = f'checkpoints/ub_{self.opt.part}_{self.opt.selected_source_hospital}_{timestr}'
...
def tsne(features_tsne, labels_tsne, level, path, name):

    sample_size=500
    indices = random.sample(range(len(features_tsne)), min(sample_size, len(features_tsne)))
    features_tsne = [features_tsne[i] for i in indices]
    labels_tsne = [labels_tsne[i] for i in indices]
    ...
    # 根据标签使用不同的颜色进行绘制
    plt.figure() # 若不新建图片, 每次会在同一个图像上画
    for i in np.unique(labels_tsne):
        plt.scatter(features_tsne[labels_tsne == i, 0], features_tsne[labels_tsne == i, 1], label=f'Class {i}', s=10, color=colors[i])
    plt.axis('off')
    # plt.title('T-SNE Visualization of ours Features')
    # plt.xlabel('T-SNE Dimension 1')
    # plt.ylabel('T-SNE Dimension 2')
    if not os.path.exists(path):
        os.makedirs(path)
    na = f'{name}_{level}.jpg'
    plt.savefig(os.path.join(path, na))
    plt.close()
