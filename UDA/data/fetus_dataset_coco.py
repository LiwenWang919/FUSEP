    ...
    def __init__(self, opt, operation='train', domain='Source'):
        ...
        if opt.part == 'heart':
            ...
            if self.operation == 'test':
                self.operation = 'val'
            anno_path = os.path.join('/media/Storage2/Lvxg/ToMo-UDA/dataset/MMWHS/', self.selected_hospital, self.operation,
                        'annotations.json')
            self.img_path = os.path.join('/media/Storage2/Lvxg/ToMo-UDA/dataset/MMWHS/', self.selected_hospital, self.operation)
        elif opt.part == 'spine':
            anno_path = os.path.join(opt.dataset_path_spine, self.selected_hospital, 
                                     self.operation+'.json')
            self.img_path = os.path.join(opt.dataset_path_spine, self.selected_hospital, 'src')
        elif opt.part == 'abdomen':
            anno_path = os.path.join(opt.dataset_path_abdo, self.selected_hospital, 
                                     self.operation+'.json')
            self.img_path = os.path.join(opt.dataset_path_abdo, self.selected_hospital, 'src')
        elif opt.part == 'EP':
            anno_path =os.path.join(opt.dataset_path_ep, self.selected_hospital, 
                                     self.operation+'.json')
            self.img_path = os.path.join(opt.dataset_path_ep, self.selected_hospital, 'src')

        elif opt.part == '4cc':
            anno_path = os.path.join(opt.dataset_path_4cc, self.selected_hospital, self.operation+'.json')
            self.img_path = os.path.join(opt.dataset_path_4cc, self.selected_hospital, 'src')
        elif opt.part == '3vt':
            anno_path = os.path.join(opt.dataset_path_3vt, self.selected_hospital, self.operation+'.json')
            self.img_path = os.path.join(opt.dataset_path_3vt, self.selected_hospital, 'src')
        elif opt.part == 'CHAOS':
            if self.operation == 'val':
                self.operation = 'test'
            anno_path = os.path.join(opt.dataset_path_chaos, self.selected_hospital, self.operation, 'annotations.json')
            self.img_path = os.path.join(opt.dataset_path_chaos, self.selected_hospital, self.operation)
        elif opt.part == 'CRL':
            if self.selected_hospital == 'SC':
                self.img_path = '/media/Storage3/wlw/NIPSDB/SCSZ/SC-tuomin-hou-anno/'
                anno_path = self.img_path + 'CRL_train.json'
                if self.operation == 'test':
                    anno_path = self.img_path + 'CRL_test.json'
            
            if self.selected_hospital == 'SZ':
                self.img_path = '/media/Storage3/wlw/NIPSDB/SCSZ/SZ-tuomin-hou-anno/'
                anno_path = self.img_path + 'CRL_train.json'
                if self.operation == 'test':
                    anno_path = self.img_path + 'CRL_test.json'
            if self.selected_hospital == 'YN':
                self.img_path = '/media/Storage3/wlw/NIPSDB/SCSZ/SZ-tuomin/Merge/'
                anno_path = self.img_path + 'CRL_train.json'
                if self.operation == 'test':
                    anno_path = self.img_path + 'CRL_test.json'
        elif opt.part == 'NT':
            if self.selected_hospital == 'SC':
                self.img_path = '/media/Storage3/wlw/NIPSDB/SCSZ/SC-tuomin-hou-anno/'
                anno_path = self.img_path + 'CNTtrain.json'
                if self.operation == 'test':
                    anno_path = self.img_path + 'NT_test.json'
            
            if self.selected_hospital == 'SZ':
                self.img_path = '/media/Storage3/wlw/NIPSDB/SCSZ/SZ-tuomin-hou-anno/'
                anno_path = self.img_path + 'NT_train.json'
                if self.operation == 'test':
                    anno_path = self.img_path + 'NT_test.json'
            if self.selected_hospital == 'YN':
                self.img_path = '/media/Storage3/wlw/NIPSDB/SCSZ/SZ-tuomin/Merge/'
