...
from adapteacher.modeling.meta_arch.cluster import DUC
...
class GModule(torch.nn.Module):
    ...
    def __init__(self, cfg, in_channels):
        ...
        self.num_classes = cfg.MODEL.ROI_HEADS.NUM_CLASSES #Lvxg

        self.with_hyper_graph = False

        # One-to-one (o2o) matching or many-to-many (m2m) matching?
        self.matching_cfg = 'o2o'  # 'o2o' and 'm2m' d:o2o     
    ...
    def _forward_train(self, features, labels, center):
        ...
        if self.matching_cfg != 'none':
            ...
            if self.with_quadratic_matching:
                ...
                matching_loss_quadratic = matching_loss_quadratic
    ...
    def _forward_cross_domain_graph(self, nodes_1, nodes_2):
        ...
        n_2 = len(nodes_2)
        if nodes_1.dim() == 1:
            nodes_1 = nodes_1.unsqueeze(0)
        if nodes_2.dim() == 1:
            nodes_2 = nodes_2.unsqueeze(0)
    ...
    def _forward_aff(self, nodes_1, nodes_2, labels_side1, labels_side2):
        ...
        elif self.matching_cfg == 'inner_product':
            M = torch.mm(nodes_1, nodes_2.t())
            matching_target = torch.mm(self.one_hot(labels_side1), self.one_hot(labels_side2).t())
            matching_loss = self.matching_loss(M.sigmoid(), matching_target.float()).mean()
            matching_loss = matching_loss * 0.01
        else:
            M = None
            matching_loss = 0
...
if __name__ == '__main__':
    ...
    #         # 分解输入的元组
    #         feats_s, feats_t = inputs[0]
    #         labels_s, labels_t = inputs[1]
    #         x = inputs[2]
    #         return self.model((feats_s, feats_t), (labels_s, labels_t), x)
    
    # graph_matching = GModule(None, 512).to("cuda:1")
    # graph_matching_wrapped = GModuleWrapper(graph_matching)
    # labels_s = torch.randint(0, 9, (40,), dtype=torch.int).to("cuda:1")
    # labels_t = torch.randint(0, 9, (51,), dtype=torch.int).to("cuda:1")
    # feats_s = torch.randn(40, 512).to("cuda:1")
    # feats_t = torch.randn(51, 512).to("cuda:1")
    # x = torch.randn(9, 512).to("cuda:1")

    # inputs = [(feats_s, feats_t), (labels_s, labels_t), x]

    # flops = FlopCountAnalysis(graph_matching_wrapped, inputs)
    ...
    deep_cluster = DUC(None).to("cuda:1")
    deep_cluster_wrapped = GModuleWrapper(deep_cluster)
    labels = torch.randint(0, 9, (40,), dtype=torch.int).to("cuda:1")
    feats = torch.randn(40, 512).to("cuda:1")
    inputs = [feats, labels, None]
    flops = FlopCountAnalysis(deep_cluster_wrapped, inputs)
    print("Total FLOPs:", flops.total())